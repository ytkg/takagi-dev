...
export const DesktopDropdown = ({ item }: { item: NavDropdown }) => {
  ...
  return (
    <div className="relative inline-block" onMouseEnter={() => setIsToolsOpen(true)} onMouseLeave={() => setIsToolsOpen(false)}>
      <button className="p-4 inline-flex items-center hover:bg-gray-100 transition-colors rounded-md">
        <span>{item.text}</span>
        ...
        <div className="absolute right-0 w-max mt-0 origin-top-right bg-white border border-gray-200 divide-y divide-gray-100 rounded-md shadow-lg">
          <div className="py-1">
            {item.subLinks.map((subLink) => (
              <Link key={subLink.to} to={subLink.to} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900 transition-colors">
                {subLink.text}
              </Link>
            ))}
          </div>
